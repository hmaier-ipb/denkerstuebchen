...
function init(){
  ...
  console.log(identifyAdmin);
  identifyAdmin() === true ? confirm_reservation[0].style.display = "block": confirm_reservation[0].style.display = "none";
...
function initEventListeners(){
  ...
  close_res.addEventListener("click", e =>{
    confirm_reservation[0].style.display = "none";
  })
...
function identifyAdmin(){
  var admin = false;
  var usernames = ["hmaier","mlanglho","hbartz","rschelle","tabe","tkoerner"];
  var div_text = document.getElementById("header").getElementsByTagName("div")[6].innerText;
  usernames.forEach(function (index){
    if(div_text === "Benutzername"+index){
      admin = true;
    }
  })
  return admin;
